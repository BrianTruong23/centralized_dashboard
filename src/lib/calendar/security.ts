...
export function decryptSecret(payload: string): string {
  ...
  const decipher = crypto.createDecipheriv('aes-256-gcm', getEncryptionKey(), fromBase64Url(ivPart), { authTagLength: 16 }); // nosemgrep: javascript.node-crypto.security.gcm-no-tag-length.gcm-no-tag-length
