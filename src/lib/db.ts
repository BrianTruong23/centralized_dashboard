...
const mapRowToTask = (row: any): Task => ({
  ...
  scheduled_start: row.scheduled_start || row.start_time || null,
  scheduled_end: row.scheduled_end || row.end_time || null,
...
function taskToRow(task: Task): Record<string, any> {
  const row: Record<string, any> = {
    ...
    scheduled_on: task.scheduled_on || task.scheduled_date || null,
    scheduled_start: task.scheduled_start || task.start_time || null,
    scheduled_end: task.scheduled_end || task.end_time || null,
    start_time: task.scheduled_start || task.start_time || null,
    end_time: task.scheduled_end || task.end_time || null,
    tags: task.tags,
